  ...
  function controller($q, nodesService, config,
    ...
    function init() {
      ...
      .then(function(responses) {
        ...
        ctrl.defaultLeaseTime = responses[1].data.default_lease_time;
    ...
    function createLease(nodes) {
      createLeasesModalService.open({ defaultLeaseTime: ctrl.defaultLeaseTime })
    ...
    function deleteLease(nodes) {
      ...
      .then(function (response) {
        ...
        var promises = [];
        angular.forEach(nodes, function(node) {
          promises.push(nodesService.deleteLease(leasesToDelete[node.uuid].uuid));
        });
